...
public class Maps {
	...
	public void fade(int changeMap, int oldMap, Graphics2D g2, int worX, int worY, int width, int height, int oldworX,
		...
		if (input.changeMapPressed && worldX >= oldworX && worldX <= oldworX + oldWidth && worldY >= oldworY
		...
		if (!fading && input.changeMapPressed && worldX >= worX && worldX <= worX + width && worldY >= worY
				&& worldY <= worY + height) {
			fading = true;
			p.disableCharacterMovement();
			stepCount = 0;
